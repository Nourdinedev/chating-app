...
module.exports.renderUser = async (req, res) => {
   ...
   const findconversation = await Conversation.findOne({
      participants: [req.user.email, contact],
   });
   const findconversation2 = await Conversation.findOne({
      participants: [contact, req.user.email],
   });
...
module.exports.renderConversation = async (req, res) => {
   ...
   const findconversation2 = await Conversation.findOne({
      participants: [contact.email, currentUser.email],
   });
   ...
   if (currentUser.contacts.length) {
      ...
      const { id } = req.params;
      const ConversationContact = await User.findById(id);
      const addUserToConverstion = await User.findByIdAndUpdate(user._id, {
